% =========================================================================
% FERRO_A: LYAPUNOV SWEEP OVER SERIES CAPACITANCE C  (Circuit A)
% =========================================================================
% Rows:    C values from Table 5.3 of the thesis
% Columns: lambda_max at t_final = 2 s and t_final = 10 s
%
% Purpose: identify transient-chaos cases (lambda_max > 0 at 2 s,
%          but lambda_max <= 0 at 10 s) across the capacitance sweep.
% =========================================================================
clear; clc; close all;
global par;

% -------------------------------------------------------------------------
% 1. FIXED PARAMETERS  (Circuit A baseline, Case 2 of the thesis)
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
% 2. SWEEP RANGE  (matches Table 5.3 in the thesis)
% -------------------------------------------------------------------------
C_vec = [0.01, 0.05, 0.08, 0.1, 0.2, 0.3, 0.35, 0.4, 0.45, 0.5, ...
         0.55, 0.6, 0.65, 0.7, 0.75, 0.77, 0.78, 0.79, 0.8, 0.85, ...
         0.9, 1.0, 1.1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, ...
         1080, 1085] * 1e-6;     % Capacitance values [F]
nC = length(C_vec);
...
fprintf('Running %d capacitance points  (2 Lyapunov runs per point)...\n', nC);
tic;
for i = 1:nC
    C = C_vec(i);

    % Derived parameters (must be recomputed every time C changes)
...
disp(' ');
disp('==============================================================');
disp(' Maximum Lyapunov exponent vs. series capacitance  (Circuit A)');
disp('==============================================================');
disp(T_lambda);
...
% 7. EXPORT CSV
% -------------------------------------------------------------------------
writetable(T_lambda, 'Circuit_A_lambda_vs_C.csv');
fprintf('Results saved to Circuit_A_lambda_vs_C.csv\n');
...
xlabel('Series capacitance C [\muF]', 'FontName', 'Times New Roman', 'FontSize', 13);
ylabel('\lambda_{max}',               'FontName', 'Times New Roman', 'FontSize', 13);
title('Maximum Lyapunov Exponent vs. Series Capacitance (Circuit A)', ...
